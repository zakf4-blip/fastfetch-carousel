...
* **Modificar fastfetch**: El único archivo que deben tocar para modificar aspectos, módulos y campos del fastfetch es **config.jsonc** en `~/.config/fastfetch/`, preferiblemente no tocar los campos de **source y type** para evitar errores en la carga aleatoria. Para agregar nuevos módulos como `date time, version, font, etc` y se siga mantiendo la sincronia de los colores se deben de crear junto con la variable `Keycolor` y darl algunos de estos valores `C1, C2, C3 o C4`.
