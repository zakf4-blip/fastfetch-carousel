...
## Como Modificar:
❗Primero considerar que debe haber la misma cantidad de opciones de colores en `colores.conf` y la misma cantidad de imagenes en `logos` para que el script `ff-random.sh` funcione correctamente.
Por si quieren quitar o agregar imagenes de la configuración también deben de tener en cuenta a `colores.conf`. La mayoría de limitaciones que hay en la modificación es debido al script `ff-random.sh`, si desean ignorar alguna limitación deben de readaptar el script `ff-random.sh` 
...
* **fastfetch**: El único archivo que deben tocar para modificar aspectos y campos del fastfetch es **config.jsonc** en `~/.config/fastfetch/`, preferiblemente no tocar los campos de **source y type** para evitar errores en la carga aleatoria. Para agregar nuevos modulos como `date time, version, font, etc` y se siga mantiendo la sincronia de los colores se deben de crear junto con la variable `Keycolor` y darl algunos de estos valores `C1, C2, C3 o C4`
